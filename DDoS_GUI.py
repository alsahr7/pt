...
def Attack_Target():
    ...
    if str(platform.system()) == 'Linux':
        os.system('figlet P.T.Y_DDos Tool')
    else:
        os.system("pyfiglet P.T.Y_DDos Tool")
    messagebox.showinfo("Attack Status", "P.T.Y_DDos Attack has been Started with " + str(threads) + " on Website " + website)
...
root = tk.Tk()
root.title("P.T.Y_DDos Tool GUI")

Information = Label(text = "P.T.Y_DDos Tool (GUI Implementation by P.T.Y Team)", font = 'Calibri')
...
Attack_Button = Button(text = 'Attack Target', font = 'Calibri', bd = 5, command = Attack_Target)
